...
// NUEVO: MAX LIVES pone 3 vidas al jugador.
const maxLives = 3;
...
window.addEventListener("keydown", (e) => {
    ...
    if(e.key === " " && gameOver){
        ...
        lives = maxLives; // resetear vidas por las máximas (3).
...
let lives = maxLives;  // pone una variable que declara que vidas existen en el juego.
        ...
        for(let i = enemies.length - 1; i >= 0; i--){
            ...
            if(rectsOverlap(px, player.y, CAR_WIDTH, CAR_HEIGHT, 
                ex, e.y, CAR_WIDTH, CAR_HEIGHT)){
                    // Funcion para decir que al hacer el jugador una colisión, quite una vida al jugador hasta que sea 0
                    enemies.splice(i, 1); // elimina al enemigo con el que chocaste
                    lives--
                    if(lives <= 0){
                        gameOver = true;
                        submitScore();
...
function render(){
    ...
    // Vidas — corazones en la esquina superior izquierda, debajo del score
    ctx.font = '16px "Courier New", monospace';
    ctx.textAlign = 'left';
    for(let i = 0; i < maxLives; i++){
        ctx.fillStyle = i < lives ? '#ff66cc' : '#333355'; // lleno o vacío
        ctx.fillText('♥', 10 + i * 22, 48);
    }
